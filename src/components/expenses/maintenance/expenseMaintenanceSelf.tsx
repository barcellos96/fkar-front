...
import { ExpenseVehicleServicesProps } from "@/providers/expense/expenseVehicle";
...
export default function ExpenseMaintenanceSelf({
  item,
  handleClose,
}: DataProps) {
        ...
        <div className="pr-7 pt-4">
          ...
          <section className="flex items-start gap-2 pt-6 pb-6 border-b">
            ...
            <section className="flex flex-col w-full">
              <span className="font-semibold text-lg mb-2">Serviços:</span>
              <ol className="flex flex-col gap-2">
                {item?.expense_vehicle_services?.map((item, index) => (
                  <li key={index} className="flex justify-between ">
                    <span className="text-base">
                      {item.expense_service.name}
                    </span>
                    <span className="text-base">
